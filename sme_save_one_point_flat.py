...
output_dir = r"c:/Users/acer/Desktop/TESI/SET_MEMB_1_D/PLOT/FINAL FOUR"
os.makedirs(output_dir, exist_ok=True)

# Define file name
file_name = os.path.join(output_dir, f"Flat_noise({n_low},{n_up})_x_point({x_fix}).gif")
...
with writer.saving(fig, file_name, dpi=100):
    for i in range(len(x)):  # Iterate over all elements
        ...
        if len(valid_a) >= 2:
            # Mantieni solo gli ultimi due valori se valid_a ha più di due elementi
            if len(valid_a) > 2:
                valid_a = valid_a[-2:]  # Mantieni gli ultimi due elementi
            ...
            a_up = valid_a[1]
            a_low = valid_a[0]
            y_min = valid_a[0] * x_fix
            y_max = valid_a[1] * x_fix
            ...
            y_point1 = valid_a[0] * x_fix
            y_point2 = valid_a[1] * x_fix
            point1.set_data([x_fix], [y_point1])
            point2.set_data([x_fix], [y_point2])
